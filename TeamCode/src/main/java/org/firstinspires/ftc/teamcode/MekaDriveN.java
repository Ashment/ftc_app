...
class NxtMotor {
    ...
    public NxtMotor(DcMotorController dcCon, int p){
        cont = dcCon;
        port = p;
    }

    public void setMode(DcMotor.RunMode m){
        cont.setMotorMode(port, m);
    }

    public void setDirection(int mu){
        mult = mu;
    }

    public void setPower(double poweer){
        cont.setMotorPower(port, poweer);
    }
}

public class  MekaDriveN extends OpMode {
    ...
    DcMotorController leftSide;
    DcMotorController rightSide;
    ...
    NxtMotor motFL, motFR, motBR, motBL;
    ...
    public MekaDriveN(DcMotorController lController, DcMotorController rController, boolean useEncoders, boolean isAutonomous) {
        MekaSetup();
        motFL = new NxtMotor(lController, 1);
        motBL = new NxtMotor(lController, 2);

        motFR = new NxtMotor(rController, 1);
        motBR = new NxtMotor(rController, 2);
    ...
    public void MekaSetup(){
        ...
        motFR.setDirection(-1);
        motFL.setDirection(-1);
        motorPolarity = 1;
